...
sudo -E go test $VERBOSE -tags "cassandra" github.com/iParadigms/walker/test -cover -coverpkg github.com/iParadigms/walker -coverprofile=test/coverage.out && \
sudo -E go tool cover -html=test/coverage.out -o test/coverage.html && \
go test $VERBOSE github.com/iParadigms/walker/dnscache -cover -coverprofile=dnscache/coverage.out && \
go tool cover -html=dnscache/coverage.out -o dnscache/coverage.html && \
go test $VERBOSE github.com/iParadigms/walker/cmd -cover -coverprofile=cmd/coverage.out && \
go tool cover -html=cmd/coverage.out -o cmd/coverage.html && \
go test $VERBOSE -tags "cassandra" github.com/iParadigms/walker/cassandra -cover -coverprofile=cassandra/coverage.out && \
